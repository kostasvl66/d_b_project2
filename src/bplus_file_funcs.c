...
// initialized, and gets the found block's handle returns 0 on success, -1
// otherwise
...
int bplus_create_file(const TableSchema *schema, const char *fileName) {
  ...
  header_temp->max_records_per_block =
      (int)((BF_BLOCK_SIZE - sizeof(DataNodeHeader)) /
            (schema->record_size + sizeof(int)));
  header_temp->max_indexes_per_block =
      1 + (int)((BF_BLOCK_SIZE - sizeof(IndexNodeHeader) - sizeof(int)) /
...
int bplus_record_insert(const int file_desc, BPlusMeta *metadata,
  ...
  int inserted_key = record_get_key(&(ctx.internal_metadata->schema),
