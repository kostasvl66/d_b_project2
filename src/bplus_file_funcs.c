...
const char BF_MAGIC_NUM[4] = {0x80, 0xAA, 'B',
                              'P'}; // this identifies the file format
...
// starting from the root block (with root_index), searches for the data block
// that could contain a record with key as PK found_block must be already
// initialized, and gets the found block's handle found_block_index gets the
// found block's index returns 0 on success, -1 otherwise
int tree_search_data_block(int root_index, int key, int file_desc,
                           BF_Block *found_block, int *found_block_index) {
  ...
  *found_block_index = root_index;
  ...
  IndexNodeHeader *block_header =
      index_block_read_header(block_start); // getting the header
  if (!block_header) {
  ...
  if (position == -1) {
    ...
    IndexNodeEntry *entry =
        index_block_read_entry(block_start, block_header, position);
    if (!entry) {
      CALL_BF(BF_UnpinBlock(found_block));
      free(block_header);
      return -1;
...
// starting from non leaf node block, which is an index block (with
// non_leaf_node_index), its min_record_key is updated to new_min; then its
// parent is found and the parent's min_record_key is also updated; this is done
// recursively up to the root temp_block must be already initialized, and should
// be destroyed after the call returns 0 on success, -1 otherwise
int bubble_up_min_record_key(int non_leaf_node_index, int new_min,
                             int file_desc, BF_Block *temp_block) {
  ...
  IndexNodeHeader *temp_block_header =
      index_block_read_header(temp_block_start);
  ...
  int parent_index =
      temp_block_header->parent_index; // storing parent index to visit next
  ...
  if (parent_index == -1)
    ...
    return bubble_up_min_record_key(parent_index, new_min, file_desc,
                                    temp_block);
}
...
int bplus_create_file(const TableSchema *schema, const char *fileName) {
  ...
  header_temp->max_records_per_block =
      (int)((BF_BLOCK_SIZE - sizeof(DataNodeHeader) - sizeof(int)) /
            (sizeof(Record) + sizeof(int)));
  header_temp->max_indexes_per_block =
      1 + (int)((BF_BLOCK_SIZE - sizeof(IndexNodeHeader) - 2 * sizeof(int)) /
                sizeof(IndexNodeEntry));
  header_temp->root_index =
      -1; // this means that the B+ tree has currenty no root

  memcpy(BF_Block_GetData(header_block), header_temp,
         sizeof(BPlusMeta)); // memcpy to avoid unaligned address problems
  free(header_temp);
...
int bplus_open_file(const char *fileName, int *file_desc,
                    BPlusMeta **metadata) {
  ...
  memcpy(temp, header_data,
         sizeof(BPlusMeta)); // memcpy to avoid alignment issues
  int magic_num_is_valid =
      (memcmp(temp->magic_num, BF_MAGIC_NUM, sizeof(BF_MAGIC_NUM)) == 0);
  free(temp);
  ...
  *metadata = malloc(sizeof(BPlusMeta));
  ...
  // The pointer receives a *copy* of the actual metadata, it does not point to
  // block 0 itself Therefore, the metadata in block 0 remains untouched and
  // independent from any other function calls This is done as a security
  // measure against metadata corruption, or errors caused by copying unaligned
  // data This *copy* is always updated when the B+_Tree File is opened with
  // this function, so there are no problems with data consistency
...
int bplus_close_file(const int file_desc, BPlusMeta *metadata) {
  ...
  // Since the metadata pointer was used with a *copy* of block 0, which was
  // independent of the Block File Structure, the pointer is freed and set to
  // NULL to avoid issues with memory allocation and pointer dangling
  free(metadata);
  metadata = NULL;
...
void cleanup_context(struct context *ctx) {
  ...
  // all the following BF_Block pointers are initialized to NULL at the very
  // start if any of them is not NULL, they have been changed by helper
  // functions and it is safe to BF_Block_Destroy them else, destroying NULL
  // block pointers is undefined, so it is avoided
  if (ctx->header_block) {
  ...
  // if any is not NULL, it has been changed by helper functions and free is
  // safe with that else, free(NULL) is also safe
...
int create_data_block_root(struct context *ctx) {
  // there is no root, so it must be made as a data block, which will store the
  // record directly
  BF_Block *root_block;
  ...
  memcpy(ctx->metadata, ctx->internal_metadata,
         sizeof(BPlusMeta)); // updating the external metadata
  ...
  if (data_block_write_unordered_record(
          root_block_start, ctx->internal_metadata, 0, ctx->record) == -1) {
  ...
  int *root_index_array =
      malloc(ctx->internal_metadata->max_records_per_block * sizeof(int));
  if (!root_index_array) {
    CALL_BF(BF_UnpinBlock(root_block));
    BF_Block_Destroy(&root_block);
    free(root_block_header);
  ...
  data_block_write_index_array(root_block_start, ctx->internal_metadata,
                               root_index_array);

  BF_Block_SetDirty(root_block);
  data_block_print(root_block_start, ctx->internal_metadata);
  CALL_BF(BF_UnpinBlock(root_block));
  BF_Block_Destroy(&root_block);
  free(root_block_header);
...
int find_matching_data_block(struct context *ctx) {
  // searching for the data block that could contain a record with inserted_key
  // as PK
  BF_Block_Init(&(ctx->found_block)); // initializing the data block that the
                                      // search will find

  if (tree_search_data_block(ctx->internal_metadata->root_index,
                             ctx->inserted_key, ctx->file_desc,
                             ctx->found_block, &(ctx->found_block_index)) == -1)
    return -1;
...
int find_data_block_insert_pos(struct context *ctx) {
  ...
  ctx->found_block_index_array = data_block_read_index_array(
      ctx->found_block_start, ctx->internal_metadata);
  if (!(ctx->found_block_index_array))
    return -1;

  // searching for the position that the record could be inserted at
  ctx->found_block_insert_pos = data_block_search_insert_pos(
      ctx->found_block_start, ctx->found_block_header,
...
int insert_record_to_data_block(struct context *ctx) {
  ...
  ctx->internal_metadata->record_count++;
  memcpy(ctx->metadata, ctx->internal_metadata,
         sizeof(BPlusMeta)); // updating external metadata

  // first writing to the end of the heap
  int heap_append_pos = ctx->found_block_header->record_count - 1;
  if (data_block_write_unordered_record(ctx->found_block_start,
                                        ctx->internal_metadata, heap_append_pos,
                                        ctx->record) == -1)
    return -1;

  // shifting the indexes of index array starting from position
  // found_block_insert_pos, to make space for the new index
  memmove(&(ctx->found_block_index_array[ctx->found_block_insert_pos + 1]),
          &(ctx->found_block_index_array[ctx->found_block_insert_pos]),
          (ctx->found_block_header->record_count - 1 -
           ctx->found_block_insert_pos) *
              sizeof(int));

  // setting index array's new free position to hold the new record's
  // heap_append_pos
  ...
  if (ctx->inserted_key < ctx->found_block_header->min_record_key) {
    ...
    // if the data block has a parent (an index node), the parent's min key must
    // also be updated that update must "bubble up" from parent to parent, to
    // the root
    if (ctx->found_block_header->parent_index != -1) {
      BF_Block *temp_block;
      BF_Block_Init(&temp_block);
      bubble_up_min_record_key(ctx->found_block_header->parent_index,
                               ctx->inserted_key, ctx->file_desc, temp_block);
  ...
  data_block_write_index_array(ctx->found_block_start, ctx->internal_metadata,
                               ctx->found_block_index_array);
...
int prepare_for_new_data_block(struct context *ctx) {
  ...
  ctx->temp_heap = malloc((ctx->internal_metadata->max_records_per_block + 1) *
                          sizeof(Record));
  if (!(ctx->temp_heap))
    return -1;

  // copying found block's heap to temp_heap, leaving the last element empty
  data_block_read_heap_as_array(ctx->found_block_start, ctx->found_block_header,
                                ctx->internal_metadata, ctx->temp_heap);

  // adding the new record to the end of the temp_heap, which is currently empty
  memcpy(&(ctx->temp_heap[ctx->internal_metadata->max_records_per_block]),
         ctx->record, sizeof(Record));

  // creating a temp_index_array with one more space than found block's index
  // array
  ctx->temp_index_array =
      malloc((ctx->internal_metadata->max_records_per_block + 1) * sizeof(int));
  if (!(ctx->temp_index_array))
    return -1;

  // copying found block's index array to temp_index_array, leaving the last
  // element empty
  memcpy(ctx->temp_index_array, ctx->found_block_index_array,
         ctx->internal_metadata->max_records_per_block * sizeof(int));

  // shifting temp_index_array's elements starting in position
  // found_block_insert_pos by one element; this makes space for the new index
  memmove(
      &(ctx->temp_index_array[ctx->found_block_insert_pos + 1]),
      &(ctx->temp_index_array[ctx->found_block_insert_pos]),
      (ctx->found_block_header->record_count - ctx->found_block_insert_pos) *
          sizeof(int));

  // adding the new record's heap index to the position found_block_insert_pos
  // of temp_index_array
  ctx->temp_index_array[ctx->found_block_insert_pos] =
      ctx->internal_metadata->max_records_per_block;

  // defining the first position of temp_index_array from which the new data
  // block (to be made) will start the first half will be larger by 1 or equal
  // to the second half
  ctx->second_half_start =
      get_ceiling((ctx->internal_metadata->max_records_per_block + 1) / 2.0f);
...
int create_new_data_block(struct context *ctx) {
  ...
  ctx->new_data_block_index_array =
      malloc(ctx->internal_metadata->max_records_per_block * sizeof(int));
...
int split_content_between_data_blocks(struct context *ctx) {
  // the first (old) data block is found_block and second (new) data block is
  // new_data_block

  int first_half_count = ctx->second_half_start;
  int second_half_count =
      (ctx->internal_metadata->max_records_per_block + 1) - first_half_count;

  // finding in which block index the inserted record is to go
  int first_key_in_second_half = record_get_key(
      &(ctx->internal_metadata->schema),
  ...
  // updating second data block (the last i is the last position in
  // temp_index_array, which has one more element)
  for (int i = ctx->second_half_start;
       i < ctx->internal_metadata->max_records_per_block + 1; i++) {
    Record *rec = &(ctx->temp_heap[ctx->temp_index_array[i]]);

    int local_i = i - ctx->second_half_start; // 0-based position for the second
                                              // block's index array and heap

    // writing to the new block's heap
    if (data_block_write_unordered_record(ctx->new_data_block_start,
                                          ctx->internal_metadata, local_i,
                                          rec) == -1)
      return -1;

    ctx->new_data_block_index_array[local_i] = local_i;
  }
  data_block_write_index_array(ctx->new_data_block_start,
  ...
  int found_block_new_min_record_key =
      record_get_key(&(ctx->internal_metadata->schema),
                     &(ctx->temp_heap[ctx->temp_index_array[0]]));
  if (found_block_old_min_record_key != found_block_new_min_record_key) {

    // min key must change both for the found_block and for its parents, up to
    // the root
    ...
    if (ctx->found_block_header->parent_index != -1) {
      ...
      BF_Block_Init(&temp_block);
      bubble_up_min_record_key(ctx->found_block_header->parent_index,
                               found_block_new_min_record_key, ctx->file_desc,
                               temp_block);
      BF_Block_Destroy(&temp_block);
    }
  }
  ...
  // writing back the headers; parent index of the new block will change in any
  // case, but still it is helpful
  data_block_write_header(ctx->found_block_start, ctx->found_block_header);
  data_block_write_header(ctx->new_data_block_start,
                          ctx->new_data_block_header);
...
int bplus_record_insert(const int file_desc, BPlusMeta *metadata,
                        const Record *record) {
  // this contains the "context variables" needed by this function;
  // it is used to pass the whole context to each helper function;
  // each helper function can update the context, so that other ones can use it
  // later
  struct context ctx = {
      0}; // all members are initialized to 0 (pointers to NULL)
  ...
  ctx.inserted_key = record_get_key(&(ctx.internal_metadata->schema),
  ...
  // find the hypothetical insert position for the new record in the matching
  // data block, even if it doesn't have free space
  SAFE_CALL(find_data_block_insert_pos(&ctx), ctx);

  // checking if the matching data block actually has free space
  if (data_block_has_available_space(ctx.found_block_header,
                                     ctx.internal_metadata)) {
  ...
  // update the old and new block with the new data
